...
function getColorsFromValue(value, decl, selectors) {
    ...
    function pushColors(v, selectors, decl) {
        ...
        if (colors.indexOf(_v) === -1) {
            colorObj[_v] = { instances: [], color: _v, colorArray: Color(v).rgbArray(), hue: Color(v).hue() };
            colors.push(_v);
        }
    ...
    var testColorName = value.match(/[a-zA-A]+/g);

    (testRgba || testColorName && testColorName.filter(function (v) {return colors.indexOf(v) > -1}) || [])
